...
<html lang="en-US">
    ...
    <body data-spy="scroll" data-target="#main-navbar">
    	...
    	<div class="body">
          ...
          <br>
					<br>
						<center><h2>Chamados Resolvidos</h2></center>
            <br>
            <center><h4>Total:<?php
            $sql = "select * from chamados where caso = 'Concluido' order by dt_fechamento DESC"; //<--- comando SQL
            $resultado = mysqli_query($conexao, $sql); //<--- consulta SQL

            $num_registros = mysqli_num_rows($resultado); //<--- contagem de registros SQL

            echo " " . $num_registros; //<--- retorno e exibição da contagem registros SQL
              ...
              <!-- Inicio tabela -->
                    ...
                    <th scope="col">EXCLUIR</th>
									...
									<?php
									$agendas=listaChamadosConcluido($conexao);
                        ...
                        <td>
                        <form action="apaga_chamado.php" method="post">
                        <input type="hidden" name="codigo" value="<?=$agenda['id']?>">
                        <button type="submit" onclick="return confirm('Tem certeza que deseja excluir este chamado?')" class="btn-danger">Excluir</button>
                          </form>
                        </td>
											</tr>
									<?php
